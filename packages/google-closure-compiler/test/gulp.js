...
describe("gulp-google-closure-compiler", function () {
  ...
  before(() => {
    originalCompilerRunMethod = Object.getOwnPropertyDescriptor(
    ...
    Object.defineProperty(ClosureCompiler.prototype, "run", {
      value: function (...args) {
        ...
        platformUtilized = /^java/.test(this.getFullCommand())
          ? "java"
          : "native";
        return retVal;
      },
  ...
  ["java", "native"].forEach((platform) => {
    describe(`${platform} version`, function () {
      ...
      const fakeFile1 = new File({
        path: `/foo-${platform}.js`,
        contents: Buffer.from('console.log("foo");'),
      });
      const fakeFile2 = new File({
        path: `/bar-${platform}.js`,
        contents: Buffer.from('console.log("bar");'),
      });

      const fixturesCompiled =
        'function log(a){console.log(a)}log("one.js");' +
      ...
      it("should emit an error for invalid flag", (done) => {
        ...
        stream.on("error", (err) => {
          err.message.should.match(
            /^(gulp-google-closure-compiler: )?Compilation error/
          );
          done();
        });
          ...
          .pipe(
            assert.first((f) => {
              f.contents
                .toString()
                .trim()
                .should.eql(fakeFile1.contents.toString());
            })
          )
          ...
          .pipe(
            assert.second((f) => {
              f.contents
                .toString()
                .trim()
                .should.eql(fakeFile2.contents.toString());
      ...
      it("should generate a sourcemap for each output file with chunks", (done) => {
        gulp
          .src([__dirname + "/fixtures/one.js", __dirname + "/fixtures/two.js"])
          .pipe(sourcemaps.init())
          .pipe(
            closureCompiler(
              {
      ...
      it("should properly compose sourcemaps when multiple transformations are chained", (done) => {
        gulp
          .src(["test/fixtures/one.js", "test/fixtures/two.js"], { base: "./" })
          .pipe(sourcemaps.init())
          .pipe(
            closureCompiler(
              {
      ...
      it("in streaming mode should emit an error", (done) => {
        gulp
          .src(__dirname + "/fixtures/**/*.js", { buffer: false })
          .pipe(
            closureCompiler(
              {
                compilation_level: "SIMPLE",
          ...
          .on("error", (err) => {
            err.message.should.match(
              /^(gulp-google-closure-compiler: )?Streaming not supported/
            );
            done();
          });
      });
